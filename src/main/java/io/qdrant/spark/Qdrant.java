...
public class Qdrant implements TableProvider, DataSourceRegister {
  ...
  /**
   * Checks if the required options are present in the provided options and chekcs
   * if the specified
   * id_field and
   * embedding_field are present in the provided schema.
   *
  ...
  void validateOptions(CaseInsensitiveStringMap options, StructType schema) {
    ...
    if (!fieldNames.contains(embeddingField)) {
      throw new IllegalArgumentException("Specified 'embedding_field' is not present in the schema");
    }
  }
}
